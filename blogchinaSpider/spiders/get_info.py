...
def get_author_info(response):
    ...
    introduce = author_page_sel.xpath('//div[@id="con_js"]/div').xpath('string(.)').extract_first().strip()
...
def get_follow_author_list(response):
    ...
    lastpagetime = follow_sel.xpath('//input[@id="lastpagetime"]/@value').extract()
        ...
        if status == 200:
            if len(comment_data['data']) > 0:
                ...
                for data in datas:
                    ...
                    replay_id = discuss_data['fids']
                    comment_item['replay_id'] = replay_id
